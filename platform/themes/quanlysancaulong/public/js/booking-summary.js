...
(function () {
  function ensureCardExists() {
    ...
    if (!card || !body) {
      ...
      var anchor = document.querySelector('#booking-summary-anchor')
  ...
  function render() {
    ...
    var html = items.map(function (it) {
      ...
      return (
        ...
        '<div>' +
        '<div class="fw-bold">' + court + ' • ' + type + '</div>' +
        '<div class="text-muted small">Ngày ' + date + ' • ' + time + '</div>' +
        '</div>' +
        '<div class="fw-bold">' + price.toLocaleString('vi-VN') + '₫</div>' +
        '</div>'
      );
    }).join('');
