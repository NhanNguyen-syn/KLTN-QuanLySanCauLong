(function () {
    ...
    const API_URL = page.dataset.apiReviews;
    const API_SUBMIT = page.dataset.apiSubmit;
    const CSRF = page.dataset.csrfToken;
    const MEMBER_NAME = page.dataset.memberName || '';
    const MEMBER_ID = page.dataset.memberId || null;
    ...
    function loadReviews() {
        let url = API_URL + '?page=' + currentPage + '&sort=' + currentSort;
    ...
    function renderReviews(reviews) {
        ...
        if (!reviews || reviews.length === 0) {
            list.innerHTML = '<div style="text-align:center;padding:3rem;color:#9ca3af;">' +
                '<svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" style="margin:0 auto 1rem;display:block;"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"></path></svg>' +
                '<p style="font-weight:600;">Chưa có đánh giá nào</p>' +
                '<p>Hãy là người đầu tiên chia sẻ trải nghiệm!</p></div>';
            return;
        }
    ...
    function renderReviewCard(review) {
        ...
        if (review.replies && review.replies.length > 0) {
            review.replies.forEach(function (reply) {
                ...
                replies += '<div class="reply-box" style="' + replyBg + '">' +
                    ...
                    deleteReplyBtn +
                    '</div>' +
                    '<p class="reply-content" style="white-space:pre-wrap;">' + escapeHtml(reply.comment) + '</p>' +
                    '</div>';
            });
        }
        ...
        var replyForm = '';
        if (MEMBER_ID) {
            replyForm = '<div id="replyBox-' + review.id + '" style="display:none;margin-top:0.75rem;">' +
                '<form class="reply-form" data-review-id="' + review.id + '">' +
                '<div style="display:flex;gap:0.5rem;">' +
                '<input type="text" class="form-input reply-input" placeholder="Viết phản hồi..." required style="flex:1;">' +
                '<button type="submit" style="padding:0.5rem 1rem;background:#065f46;color:white;border:none;border-radius:0.5rem;cursor:pointer;white-space:nowrap;">Gửi</button>' +
                '</div></form></div>';
        }

        var deleteBtn = '';
        if (MEMBER_ID && review.member_id == MEMBER_ID) {
            deleteBtn = '<button class="btn-delete-review review-action-btn" data-id="' + review.id + '" style="position:absolute;top:1rem;right:1rem;color:#ef4444;">' +
        ...
        return '<div class="review-card" style="position:relative;">' +
            deleteBtn +
            ...
            '<button class="review-action-btn btn-helpful ' + (review.is_voted ? 'active' : '') + '" data-id="' + review.id + '">' +
            '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="' + (review.is_voted ? '#059669' : 'none') + '" stroke="currentColor" stroke-width="2"><path d="M14 9V5a3 3 0 0 0-3-3l-4 9v11h11.28a2 2 0 0 0 2-1.7l1.38-9a2 2 0 0 0-2-2.3zM7 22H4a2 2 0 0 1-2-2v-7a2 2 0 0 1 2-2h3"></path></svg>' +
            '<span>Hữu ích (' + (review.helpful || 0) + ')</span>' +
            '</button>' +
            (MEMBER_ID ? '<button class="review-action-btn btn-reply-toggle" data-id="' + review.id + '"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"></path></svg>Trả lời</button>' : '') +
            '</div>' +
            replies +
            replyForm +
            '</div>';
    ...
    function renderPagination(pag) {
        ...
        var html = '<button class="pagination-btn" ' + (pag.current_page <= 1 ? 'disabled' : '') + ' data-page="' + (pag.current_page - 1) + '">' +
            '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="15 18 9 12 15 6"></polyline></svg></button>';

        for (var i = 1; i <= pag.last_page; i++) {
            html += '<button class="pagination-page ' + (i === pag.current_page ? 'active' : '') + '" data-page="' + i + '">' + i + '</button>';
        }

        html += '<button class="pagination-btn" ' + (pag.current_page >= pag.last_page ? 'disabled' : '') + ' data-page="' + (pag.current_page + 1) + '">' +
            '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="9 18 15 12 9 6"></polyline></svg></button>';
    ...
    function renderUploadedImages() {
        ...
        uploadedImages.forEach(function (src, idx) {
            ...
            wrapper.innerHTML = '<img src="' + src + '" alt="Upload">' +
                '<button type="button" class="remove-image-btn" data-idx="' + idx + '">×</button>';
